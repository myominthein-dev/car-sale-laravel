...
@section('content')
    <div class="pt-6 px-4">
        ...
        <div class="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">Manage states in the system</p>
            </div>
        ...
        <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
            <div class="sm:flex sm:items-center sm:justify-between mb-4">
                <div class="flex items-center gap-x-3">
                    <h3 class="text-lg font-semibold text-gray-900">All States</h3>
                    <span class="px-3 py-1 text-xs text-green-600 bg-green-100 rounded-full">{{ $states->count() ?? 0 }}
                        Total</span>
                </div>
                <div class="mt-3 sm:mt-0">
                    <form action="{{ route('states.index') }}" method="GET" class="flex    items-center">
                    ...
                    <tbody>
                        @forelse($states ?? [] as $state)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td class="px-4 py-3">{{ $state->id }}</td>
                                <td class="px-4 py-3 font-medium text-gray-900">{{ $state->name }}</td>
    ...
    <!-- Add State Modal -->
    <div id="add-state-modal" tabindex="-1" aria-hidden="true"
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <form id="editForm" method="POST">
                        ...
                        <div class="p-6 space-y-6">
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6">
                                    <label for="edit-name-{{ $state->id }}"
                                        class="block mb-2 text-sm font-medium text-gray-900">Name</label>
                                    <input type="text" name="name" id="edit-name" value="{{ $state->name }}"
                                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
                                        required>
                                </div>
...
@push('scripts')
    <script>
        ...
        // Add Modal
        const addModal = document.getElementById('add-state-modal');
        ...
        // Edit Modal
        const editModal = document.getElementById('edit-state-modal');
        ...
        editButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                editForm.action = `{{ route('states.update','') }}/${id}`;
        ...
        const searchForm = document.querySelector('form[action = "{{ route('states.index') }}"]')
