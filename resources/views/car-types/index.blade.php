...
@section('content')
    <div class="pt-6 px-4">
        ...
        <div class="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between">
            ...
            <div>
                ...
                <h1 class="text-2xl font-semibold text-gray-900">Car Types</h1>
                <p class="mt-1 text-sm text-gray-600">Manage vehicle car types in the system</p>
            </div>

            <button id="openAddModal"
                class="mt-4 md:mt-0 text-white bg-blue-600 hover:bg-blue-700  font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                Add Car Type
            </button>
        </div>
        ...
        <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
            <div class="sm:flex sm:items-center sm:justify-between mb-4">
                <div class="flex items-center gap-x-3">
                    <h3 class="text-lg font-semibold text-gray-900">All Car Types</h3>
                    <span class="px-3 py-1 text-xs text-green-600 bg-green-100 rounded-full">{{ $carTypes->count() ?? 0 }}
                        Total</span>
                </div>
                    ...
                    <tbody>
                        @forelse($carTypes ?? [] as $carType)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                <td class="px-4 py-3">{{ $carType->id }}</td>
                                <td class="px-4 py-3 font-medium text-gray-900">{{ $carType->name }}</td>
                                <td class="px-4 py-3 flex items-center space-x-2">
                                    <button type="button" class="edit-btn font-medium text-blue-600 hover:underline"
                                        data-id="{{ $carType->id }}" data-name="{{ $carType->name }}">
                                        Edit
                                    </button>
    ...
    <!-- Add Car Type Modal -->
    <div id="add-car-type-modal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <form action="{{ route('car-types.store') }}" method="POST">
                    ...
                    <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200">
                        <button type="submit"
                            class="text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Save</button>
                        <button type="button"
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <form id="editForm" method="POST">
                        ...
                        <div class="p-6 space-y-6">
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6">
                                    <label for="edit-name-{{ $carType->id }}"
                                        class="block mb-2 text-sm font-medium text-gray-900">Name</label>
                                    <input type="text" name="name" id="edit-name" value="{{ $carType->name }}"
                                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
                                        required>
                                </div>
                        ...
                        <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200">
                            <button type="submit"
                                class="text-white bg-blue-700 hover:bg-blue-800 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center">Update</button>
                            <button type="button"
                                class="text-gray-500 bg-white hover:bg-gray-100 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10"
...
@push('scripts')
    <script>
        ...
        // Add Modal
        const addModal = document.getElementById('add-car-type-modal');
        ...
        // Edit Modal
        const editModal = document.getElementById('edit-car-type-modal');
        ...
        editButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                editForm.action = `{{ route('car-types.update', '') }}/${id}`;
