...
<div class="pt-6 px-4">
    ...
    <div class="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">City</h1>
            <p class="mt-1 text-sm text-gray-600">Manage cities  in the system</p>
        </div>
    ...
    <div class="bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8">
        <div class="sm:flex sm:items-center sm:justify-between mb-4">
            <div class="flex items-center gap-x-3">
                <h3 class="text-lg font-semibold text-gray-900">All Cities</h3>
                <span class="px-3 py-1 text-xs text-green-600 bg-green-100 rounded-full">{{ $cities->count() ?? 0 }} Total</span>
            </div>
            <div class="mt-3 sm:mt-0">
                <form action="{{ route('cities.index') }}" method="GET" class="flex    items-center">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 flex items-center space-x-2">
                            ...
                            <form action="{{ route('cities.destroy', $city->id) }}" method="POST" class="inline-block delete-form">
                                @csrf
                                @method('DELETE')
                    ...
                    <tr class="bg-white border-b">
                        <td colspan="5" class="px-4 py-3 text-center">No car city found</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<!-- Add Fuel Type Modal -->
<div id="add-city-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center p-4 w-full md:inset-0 h-modal md:h-full">
...
<!-- Edit City Modal -->
<div id="edit-city-modal" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center p-4 w-full md:inset-0 h-modal md:h-full">
    <div class="relative w-full mx-auto max-w-2xl h-full md:h-auto">
        <div class="relative bg-white rounded-lg shadow">
            <div class="flex justify-between items-start p-4 rounded-t border-b">
                <h3 class="text-xl font-semibold text-gray-900">
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-6 gap-6">
                        <div class="col-span-6">
                            ...
                            <input type="text" name="name" id="edit-name" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  block w-full p-2.5" required>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">State</label>
                        <select name="state" id="state-edit"
...
<script>
    ...
    // Add Modal
    const addModal = document.getElementById('add-city-modal');
    ...
    // Edit Modal
    const editModal = document.getElementById('edit-city-modal');
    const closeEditModalBtn = document.getElementById('closeEditModal');
    const cancelEditBtn = document.getElementById('cancelEdit');
    const editForm = document.getElementById('editForm');
    const editNameInput = document.getElementById('edit-name');
    const editStateSelect = document.getElementById('state-edit');
    ...
    editButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            editForm.action = `{{ route('cities.update','') }}/${id}`;
    ...
    const clearBtn = document.getElementById('clearBtn');
        const searchForm = document.querySelector('form[action="{{ route("cities.index") }}"]');
